...
BEGIN
    ...
    -- Validation: Only validate phone if provided
    IF length(v_clean_phone) > 0 AND (length(v_clean_phone) < 10 OR length(v_clean_phone) > 15) THEN
    ...
    -- Upsert guest record in CRM table for the restaurant organization if phone provided
    ...
    IF p_session_id IS NOT NULL THEN
        ...
        SET guest_id = COALESCE(v_guest_id, guest_id),
            customer_name = v_clean_name,
            customer_phone = CASE WHEN length(v_clean_phone) >= 10 THEN v_clean_phone ELSE customer_phone END,
            updated_at = NOW()
