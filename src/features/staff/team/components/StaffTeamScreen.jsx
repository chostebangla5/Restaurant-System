...
export function StaffTeamScreen() {
  const { venueId, orgId, role: currentUserRole } = useAuth();
  ...
  const handleDelete = async (member) => {
    if (!window.confirm(`Remove ${member.full_name} from this venue?`)) return;
    try {
  ...
  const handleRoleChange = async (member, newRole) => {
    try {
      ...
      toast.success(`${member.full_name} role updated to ${ROLE_CONFIG[newRole]?.label}`);
    } catch (err) {
  ...
  // Summary stat cards (PetPooja-style)
  const statCards = [
    {
      title: 'Total Staff',
      value: totalStaff,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-0.5">
            Manage your team, roles, access & attendance
          </p>
        </div>
        <Button
          size="sm"
      ...
      {/* Role Breakdown Bar (PetPooja-style) */}
      ...
      {/* Staff List (PetPooja-style cards) */}
      {isLoading ? (
        ...
        <EmptyState
          icon={Users}
          title={searchQuery || filterRole !== 'all' ? 'No matching staff found' : 'No staff members yet'}
          description={
        ...
        <div className="space-y-3">
          <AnimatePresence mode="popLayout">
            {filtered.map((member, idx) => {
              ...
              const isOnline = member.is_active && new Date(member.last_active) > new Date(Date.now() - 60 * 60 * 1000);
              ...
              return (
                <motion.div
                  ...
                  className={`group relative rounded-2xl border bg-white dark:bg-stone-900 overflow-hidden transition-all hover:shadow-lg ${
                    member.is_active
                ...
                >
                  ...
                  <div className="flex items-center gap-4 px-5 py-4 pl-6">
                    ...
                    <div className="relative flex-shrink-0">
                      ...
                      {/* Online indicator */}
                      {isOnline && (
                        <span className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-emerald-500 border-2 border-white dark:border-stone-900">
                          <span className="absolute inset-0.5 rounded-full bg-emerald-400 animate-ping" />
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5">
                        ...
                        <Badge
                          size="sm"
                          className={`${config.bgLight} ${config.textColor} border ${config.borderColor}`}
                        >
                    ...
                    <div className="hidden sm:flex items-center gap-6 flex-shrink-0">
                      {/* Orders stat */}
                      {member.orders_handled_today > 0 && (
                        <div className="text-center">
                          <p className="text-lg font-black text-stone-900 dark:text-white">
                            {member.orders_handled_today}
                          </p>
                          <p className="text-[10px] text-stone-400 font-medium">Orders Today</p>
                        </div>
                      )}
                          ...
                          {openMenuId === member.id && (
                            <>
                              ...
                              <motion.div
                                ...
                                className="absolute right-0 top-full mt-1 z-20 w-48 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl shadow-xl overflow-hidden"
                              >
                                ...
                                {Object.entries(ROLE_CONFIG).map(([roleKey, roleConf]) => (
                                  ...
                                  >
                                    ...
                                    {roleConf.label}
                                    {member.role === roleKey && <span className="ml-auto text-[10px]">✓</span>}
                                  </button>
                                ))}
                                <div className="border-t border-stone-100 dark:border-stone-700">
                                  <button
                                    ...
                                    className="w-full text-left px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 flex items-center gap-2"
                                  >
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Add Staff Member" description="Invite a new team member to your venue" size="md">
      <form onSubmit={handleSubmit} className="space-y-5">
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-left ${
                    isSelected
                      ? `${config.bgLight} ${config.borderColor} ${config.textColor} shadow-sm`
        ...
        <Input
          ...
          error={errors.email}
          helperText="Staff member will use this email to log in"
        />
